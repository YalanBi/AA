#P2-Create subset of files
setwd("X:/Data/Desktop/Arabidopsis Arrays/Original Data")
arr <- read.table("exp_ann30K.txt")
#with formula which() <- not correct enough, not able to distinguish introns from intergenic
intergenic_ind <- NULL
for(x in 1:nrow(arr)){
	if(x %in% which(arr[,7] == "intergenic")){
		intergenic_ind <- c(intergenic_ind, x)
	}
}
write.table(arr[intergenic_ind,],file="intergenic.txt",sep="\t")
