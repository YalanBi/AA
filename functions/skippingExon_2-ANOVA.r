...
#splicingTestSE(filename, goal="skippingExon", verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestSE(filename, goal="skippingExon")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
  ...
  if(!is.null(resmatrix)){
    ...
    write.table(resmatrix, file=paste0("Data/AS/SE_chr", chr, "_ANOVA_less.txt"), sep="\t") #********** change!!! **********#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
