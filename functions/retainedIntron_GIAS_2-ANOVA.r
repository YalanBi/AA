...
#splicingTestRI_AS(filename, toTest="QTL", threTest=8, whichTest=wilcox.test, alternative="less", verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestRI_AS(filename, toTest="QTL", threTest=8)
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
  ...
  if(!is.null(resmatrix)){
    ...
    write.table(resmatrix, file=paste0("Data/geneticsAS/RI_chr", chr, "_GAS_ANOVA_less.txt"), sep="\t") #********** change!!! **********#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTestRI_AS(filename, toTest="Int", threTest=11.6)
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      cat(filename, "is tested\n")
    }
  ...
  if(!is.null(resmatrix)){
    colnames(resmatrix) <- c("sepProbe", "sigMarker", "6H/gt1", "6H/gt2", "Dry_AR/gt1", "Dry_AR/gt2", "Dry_Fresh/gt1", "Dry_Fresh/gt2", "RP/gt1", "RP/gt2")
    write.table(resmatrix, file=paste0("Data/geneticsAS/RI_chr", chr, "_IAS_ANOVA_less.txt"), sep="\t") #********** change!!! **********#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
  cat("chr", chr, "finished in", et-st, "s\n\n")
}
