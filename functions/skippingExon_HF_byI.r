...
testSEbyI <- function(filename, threTest=11.6, P=3, verbose=FALSE){
  ...
  if(length(uniqueExon) < 2){
    ...
    for(tu in uniqueExon){
      ...
      if(length(ind) >= P && any(apply(tuInt >= threTest, 2, sum) > 0)){
...
#testSEbyI(filename, threTest=11.6, P=3, verbose=TRUE)

for(chr in 1:5){
  st <- proc.time()[3]
  cat("chr", chr, "starts testing Int regulated skipping exons...\n")
  ...
  for(filename in genenames){
    ...
    resmatrix <- rbind(resmatrix, testSEbyI(filename, threTest=11.6, P=3))
  ...
  write.table(resmatrix, file=paste0("Data/geneticsAS/skippingExonByI_chr", chr, "_wt_p3.txt"), sep="\t")
