...
#main idea: expGenes >= 2 exons, >= 2P exonProbes
#           each exons >= 2 probes
...
splicingGorISE <- function(filename, goal="QTL", threTest=8, fnPart, P=3, verbose=FALSE){
  ...
  if(length(uniqueExon) >= 2 && length(exonID) >= 2*P){
    ...
    if(verbose) cat("sig", goal, "test starts...\n")
    testTUandTM <- NULL
    for(tu in uniqueExon){
      ind <- exonID[rawexp[exonID, "tu"] == tu]
      if(length(ind) >= P && max(fnPart[paste0(filename, "_", tu), ]) >= threTest){
        m <- which.max(fnPart[paste0(filename, "_", toTest), ])
        if(verbose) cat("\t", tu, "has p", ind, "and sig", goal, "@ marker", m, "\n")
        testTUandTM <- rbind(testTUandTM, c(tu, m))
      } else if(verbose) cat("\t", tu, "quit T^T\n")
    }
    
    if(!is.null(testTUandTM)){
      if(verbose) cat("after sig", goal, "test,", testTUandTM[ ,1], "left and start SE test in each Env and in each gt separately...\n")
      
      for(n in testTUandTM[ ,1]){
        testProbes <- exonID[rawexp[exonID, "tu"] == testTUandTM[n, 1]]
        m <- testTUandTM[n, 2]
        #************************************************************************ H E R E ! ! ! *************************************************************************#
        for(env in 1:4){
          ...
          for(gt in 1:2){
            gtInd <- which(geno[ ,m] == gt)
            
            for(tu in uniqueExon[uniqueExon != toTest]){
              ind <- exonID[rawexp[exonID, "tu"] == tu]
