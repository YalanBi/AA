...
#splicingTestRI(filename, P=2, whichTest=wilcox.test, alternative="less", verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestRI(filename, P=2, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
  ...
  if(!is.null(resmatrix)){
    ...
    write.table(resmatrix, file=paste0("Data/AS/RI_chr", chr, "_wt_less.txt"), sep="\t") #***************************** change with goal!!! *****************************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
