...
i5asMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/5'AS_chr", chr, "_IAS_ANOVA_less.txt"))){
    i5asMatrix <- rbind(i5asMatrix, read.table(paste0("Data/geneticsAS/5'AS_chr", chr, "_IAS_ANOVA_less.txt"), row.names=NULL))
  } else cat("chr", chr, "NO test!\n")
}
#Bonferroni correction
-log10(0.05/nrow(i5asMatrix)/4)# = NA; 0 first exons were tested * 4 Env
rm(i5asMatrix)
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/5'AS_chr", chr, "_IAS_ANOVA_less.txt"))){
    i5aschr <- read.table(paste0("Data/geneticsAS/5'AS_chr", chr, "_IAS_ANOVA_less.txt"), row.names=NULL)
    i5asGeneList <- list()
    resmatrix <- NULL
    ...
    if(length(gn_mixEnv) > 0) save(i5asGeneList, file = paste0("Data/geneticsAS/5'AS_chr", chr, "_IAS_gnList_ANOVA.Rdata"))
