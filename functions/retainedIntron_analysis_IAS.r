...
nrow(iriMatrix)# = 1401 exons were tested
-log10(0.05/nrow(iriMatrix)/4)# 1401 exons were tested * 4 Env; => iriThre=5.05
length(unique(iriMatrix[,1]))# = 1401 genes were tested
#STOP!
...
iriThre=round(-log10(0.05/nrow(iriMatrix)/4), digits=2)# =2.51
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/IRI_chr", chr, "_", whichTest, "_less.txt"))){
    irichr <- read.table(paste0("Data/geneticsAS/IRI_chr", chr, "_", whichTest, "_less.txt"), row.names=NULL)
    iriGeneList <- list()
    resmatrix <- NULL
    ...
    if(length(gn_mixEnv) > 0) save(iriGeneList, file=paste0("Data/geneticsAS/IRI_chr", chr, "_", whichTest, ".Rdata"))
  } else{
    cat("chr", chr, "NO test!\n")
