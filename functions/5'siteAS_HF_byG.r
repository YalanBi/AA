...
#*********************************************** this is the final version for testing genetics regulated AS at 5 site ^_^ ***********************************************#
...
#5'site GAS test
test5siteGAS <- function(filename, threTest=8, P=2, verbose=FALSE){
  ...
  #for genetics regulated alternative splicing 5'site, at least 2 exons in a gene!!!
...
#test5siteGAS(filename, threTest=8, P=2, verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- test5siteGAS(filename, threTest=8, P=2)
  ...
  write.table(resmatrix, file=paste0("Data/geneticsAS/splicing5'siteByG_chr", chr, "_wt_p2.txt"), sep="\t")
  et <- proc.time()[3]
