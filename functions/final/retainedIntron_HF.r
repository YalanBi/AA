...
testDffBtwParts <- function(exp_data=rawexp[ ,ind_env+16], testProbes, restProbes, verbose=FALSE){
  ...
  return(wilcox.test(testPart, restPart, alternative="less")$p.value)
}
...
testRI <- function(filename, P=2, verbose=FALSE){
  ...
  rownamez <- paste(filename, uniqueIntron,sep='_')
...
for(chr in 1:5){
  ...
  roRem <- which(apply(apply(resmatrix,1,is.na),2,sum)==4)
  resmatrix <- resmatrix[-roRem,]
  write.table(resmatrix, file=paste0("Data/AS/retainedIntron_chr", chr, "_wt_p2-NA_DA.txt"), sep="\t") #***************************** change with goal !!! *****************************#
  et <- proc.time()[3]
