...
testRIbyG <- function(filename, threTest=8, P=2, verbose=FALSE){
  ...
  if(length(uniqueIntron) > 0){
    ...
    for(intron in uniqueIntron){
      ...
      if(length(inPrb) >= P && any(apply(tuQTL >= threTest, 2, sum) > 0)){
...
#testRIbyG(filename, threTest=8, P=2, verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    resmatrix <- rbind(resmatrix, testRIbyG(filename, threTest=8, P=2))
  ...
  write.table(resmatrix, file=paste0("Data/geneticsAS/retainedIntronByG_chr", chr, "_wt_p2.txt"), sep="\t")
