...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/", gaol, "_chr", chr, "_", whichTest, "_less.txt"))){
    gasMatrix <- rbind(gasMatrix, read.table(paste0("Data/geneticsAS/", gaol, "_chr", chr, "_", whichTest, "_less.txt"), row.names=NULL))
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/", gaol, "_chr", chr, "_", whichTest, "_less.txt"))){
    gaschr <- read.table(paste0("Data/geneticsAS/", gaol, "_chr", chr, "_", whichTest, "_less.txt"), row.names=NULL)
    gasGeneList <- list()
    resmatrix <- NULL
    ...
    if(length(gn_mixEnv) > 0) save(gasGeneList, file = paste0("Data/geneticsAS/", gaol, "_chr", chr, "_gnList_", whichTest, ".Rdata"))
