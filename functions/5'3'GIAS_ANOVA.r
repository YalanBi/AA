...
splicingTestGI35 <- function(filename, goal="5'AS", P=2, toTest="QTL", threTest=8, verbose=FALSE){
  ...
  #for 5'/3' IAS, at least 2 exons in a gene!!!
  if(length(uniqueExon) >= 2 && length(exonID) >= (2*P+1)){
    ...
    if(length(ind) >= 2*P){
      ...
      if(length(testProbes) >= P && length(ind[-(1:sepPoint)]) >= P && any(apply(testQTL[testProbes, ] >= threTest, 2, sum) >= P)){
        ...
        #      in 5'IAS, it is the last probe of higher part in the first exon; in 3'IAS, it is the last probe of the lower part in the last exon
...
#splicingTestGI35(filename, goal="5'AS", P=2, toTest="QTL", threTest=8, verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestGI35(filename, goal="5'AS", P=2, toTest="QTL", threTest=8)
    if(!is.null(res)){
  ...
  if(!is.null(resmatrix)){
    ...
    write.table(resmatrix, file=paste0("Data/geneticsAS/5'GAS_chr", chr, "_ANOVA_less.txt"), sep="\t") #********************** change with goal!!! **********************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTestGI35(filename, goal="3'AS", P=2, toTest="QTL", threTest=8)
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      rownameList <- c(rownameList, filename)
      cat(filename, "is tested\n")
  ...
  if(!is.null(resmatrix)){
    rownames(resmatrix) <- rownameList
    colnames(resmatrix) <- c("sepProbe", "sigMarker", "6H/gt1", "6H/gt2", "Dry_AR/gt1", "Dry_AR/gt2", "Dry_Fresh/gt1", "Dry_Fresh/gt2", "RP/gt1", "RP/gt2")
    write.table(resmatrix, file=paste0("Data/geneticsAS/3'GAS_chr", chr, "_ANOVA_less.txt"), sep="\t") #********************** change with goal!!! **********************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
  cat("chr", chr, "finished in", et-st, "s\n\n")
}
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTestGI35(filename, goal="5'AS", P=2, toTest="Int", threTest=11.6)
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      rownameList <- c(rownameList, filename)
      cat(filename, "is tested\n")
  ...
  if(!is.null(resmatrix)){
    rownames(resmatrix) <- rownameList
    colnames(resmatrix) <- c("sepProbe", "sigMarker", "6H/gt1", "6H/gt2", "Dry_AR/gt1", "Dry_AR/gt2", "Dry_Fresh/gt1", "Dry_Fresh/gt2", "RP/gt1", "RP/gt2")
    write.table(resmatrix, file=paste0("Data/geneticsAS/5'IAS_chr", chr, "_ANOVA_less.txt"), sep="\t") #********************** change with goal!!! **********************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
  cat("chr", chr, "finished in", et-st, "s\n\n")
}
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTestGI35(filename, goal="3'AS", P=2, toTest="Int", threTest=11.6)
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      rownameList <- c(rownameList, filename)
      cat(filename, "is tested\n")
  ...
  if(!is.null(resmatrix)){
    rownames(resmatrix) <- rownameList
    colnames(resmatrix) <- c("sepProbe", "sigMarker", "6H/gt1", "6H/gt2", "Dry_AR/gt1", "Dry_AR/gt2", "Dry_Fresh/gt1", "Dry_Fresh/gt2", "RP/gt1", "RP/gt2")
    write.table(resmatrix, file=paste0("Data/geneticsAS/3'IAS_chr", chr, "_ANOVA_less.txt"), sep="\t") #********************** change with goal!!! **********************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
  cat("chr", chr, "finished in", et-st, "s\n\n")
}
