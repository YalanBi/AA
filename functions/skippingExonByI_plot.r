...
#plotExpEnvSep("AT1G01800", toTest="Int", testThre=3.80, fnPart, m=2)


#plot all genes having ISE
testThre=3.80
for(chr in 1:5){
  load(file=paste0("Data/geneticsAS/ISE_chr", chr, "_wt_p3.Rdata"))
  genenames <- iseGeneList[[5]]
  for(filename in genenames){
    cat(filename, "starts plotting ...\n")
    
    isechr <- read.table(paste0("Data/geneticsAS/ISE_chr", chr, "_wt_p3.txt"), row.names=NULL)
    fnPart <- isechr[grepl(filename, isechr[ ,1]), ]
    ...
    for(m in markerRange){
      png(filename = paste0("Data/geneticsAS/Plot/ISE/", filename, "_ISE_m", m, "_wt.png"), width = 960, height = 1728, bg = "white")
      plotExpEnvSep(filename, toTest="Int", testThre=3.80, fnPart, m)
      dev.off()
    }
  }
}
...
for(chr in 1:5){
  load(file=paste0("Data/geneticsAS/ISE_chr", chr, "_wt_p3.Rdata"))
  genenames <- c(genenames, iseGeneList[[5]])
...
resList <- NULL
for(e in 1:nrow(iseMatrix)){
  res <- 1
  for(env in 1:4){
    res <- res*abs(iseMatrix[e, 2*env+2]-iseMatrix[e, 2*env+3])
  }
  resList <- c(resList, res)
}
sigOrder <- unique(iseMatrix[order(resList, decreasing=TRUE), 1])
...
for(filename in mostSig[1:5]){
  ...
  cat(markerRange, "\n")
  for(m in markerRange){
    png(filename = paste0("Data/geneticsAS/Plot/ISE/", filename, "_ISE_m", m, "_wt_mS.png"), width = 960, height = 1728, bg = "white")
    plotExpEnvSep(filename, toTest="Int", testThre=3.80, fnPart, m)
    dev.off()
  }
}
...
for(e in 1:nrow(iseMatrix)){
  for(gt in 4:11){
    if(iseMatrix[e, gt] > (testThre-0.1) && iseMatrix[e, gt] < (testThre+0.1)) res <- c(res, iseMatrix[e, 1])
  }
}
...
for(filename in aroundThre[c(1:3, 9)]){
  ...
  cat(markerRange, "\n")
  for(m in markerRange){
    png(filename = paste0("Data/geneticsAS/Plot/ISE/", filename, "_ISE_m", m, "_wt_aT.png"), width = 960, height = 1728, bg = "white")
    plotExpEnvSep(filename, toTest="Int", testThre=3.80, fnPart, m)
    dev.off()
  }
}
