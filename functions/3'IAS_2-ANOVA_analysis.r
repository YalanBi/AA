...
i3asMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/3'AS_chr", chr, "_IAS_ANOVA_less.txt"))){
    i3asMatrix <- rbind(i3asMatrix, read.table(paste0("Data/geneticsAS/3'AS_chr", chr, "_IAS_ANOVA_less.txt"), row.names=NULL))
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/3'AS_chr", chr, "_IAS_ANOVA_less.txt"))){
    i3aschr <- read.table(paste0("Data/geneticsAS/3'AS_chr", chr, "_IAS_ANOVA_less.txt"), row.names=NULL)
    i3asGeneList <- list()
    resmatrix <- NULL
    ...
    if(length(gn_mixEnv) > 0) save(i3asGeneList, file = paste0("Data/geneticsAS/3'AS_chr", chr, "_IAS_gnList_ANOVA.Rdata"))
