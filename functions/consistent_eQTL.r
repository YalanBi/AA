...
#******************************************************* this is the final version for testing consistent eQTL! ^_^ ******************************************************#
...
consistent_eQTL <- function(chr, fnQTL, qtlThre=8, fnInt, intThre=11.6, fnPos, verbose=FALSE){
  res <- NULL
  for(e in 1:nrow(fnQTL)){
    ...
    if(max(fnQTL[e, ]) >= qtlThre && fnInt[e, topM] < intThre){
      ...
      if(verbose) cat("\tnot sig eQTL T^T, quit...\n")
      return(NULL)
    }
  }
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    ...
    res <- consistent_eQTL(chr, fnQTL, qtlThre=8, fnInt, intThre=11.6, fnPos)
    if(!is.null(res)){
      if(res == "cis") consistentQTL$cis <- c(consistentQTL$cis, filename)
      if(res == "trans") consistentQTL$trans <- c(consistentQTL$trans, filename)
