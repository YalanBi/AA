...
splicingTestSE <- function(filename, goal="cassetteExon", verbose=FALSE, ...){
...
#splicingTestSE(filename, goal="cassetteExon", useForTest=unlist, whichTest=wilcox.test, alternative="less", verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestSE(filename, goal="cassetteExon", useForTest=unlist, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
