...
riMatrix <- NULL
for(chr in 1:5){
  riMatrix <- rbind(riMatrix, read.table(paste0("Data/AS/RI_chr", chr, "_ANOVA_less.txt"), row.names=NULL))
}
...
for(chr in 1:5){
  richr <- read.table(paste0("Data/AS/RI_chr", chr, "_ANOVA_less.txt"), row.names=NULL)
  riGeneList <- list()
  ...
  save(riGeneList, file = paste0("Data/AS/RI_chr", chr, "_gnList_ANOVA.Rdata"))
}
