...
nrow(griMatrix)# = 1401 exons were tested 
-log10(0.05/nrow(griMatrix)/4)# 1401 exons were tested * 4 Env; => griThre=5.05
length(unique(griMatrix[,1]))# = 1401 genes were tested

#calculate the numbers of NOT sig introns and genes
griThre=round(-log10(0.05/nrow(griMatrix)/4), digits=2)# =5.05
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/GRI_chr", chr, "_", whichTest, "_less.txt"))){
    ...
    if(length(gn_mixEnv) > 0) save(griGeneList, file=paste0("Data/geneticsAS/GRI_chr", chr, "_", whichTest, ".Rdata"))
  } else{
    cat("chr", chr, "NO test!\n")
