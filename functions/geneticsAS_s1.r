    ...
    for(env in 1:4){
      ...
      for(ce in CEinEnv){
        ...
        if(length(consSigQTLMarkers) > 0){
          genesGAS[[paste0("chr", chr)]][[env]] <- c(genesGAS[[paste0("chr", chr)]][[env]], paste0(filename, "_", ce, "_", consSigQTLMarkers))
          cat("  in env", env, ",", filename, "'s", ce, "has main eQTL on marker", consSigQTLMarkers, "remember me!!!\n")
        } #else cat("  in env", env, ",", filename, "'s", ce, "has noooooooooo main eQTL T^T\n")
        if(length(consSigIntMarkers) > 0){
          genesIAS[[paste0("chr", chr)]][[env]] <- c(genesIAS[[paste0("chr", chr)]][[env]], paste0(filename, "_", ce, "_", consSigIntMarkers))
          cat("  in env", env, ",", filename, "'s", ce, "has Int QTL on marker", consSigIntMarkers, ", remember me!!!\n")
...
#*************************************************************** save part ***************************************************************#
save(genesGAS, genesIAS, file=paste0("Data/geneticsAS/genelist_QTL", threshold_qtl, "_Int",threshold_int, "_np", cutoffnProbe, ".Rdata"))
