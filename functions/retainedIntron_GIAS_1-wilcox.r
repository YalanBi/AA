...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestRI_AS(filename, toTest="QTL", threTest=8, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTestRI_AS(filename, toTest="Int", threTest=11.6, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      cat(filename, "is tested\n")
    }
