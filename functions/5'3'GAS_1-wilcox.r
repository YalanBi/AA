...
#splicingTest35_AS(filename, goal="5'AS", toTest="QTL", threTest=8, whichTest=wilcox.test, alternative="less", verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTest35_AS(filename, goal="5'AS", toTest="QTL", threTest=8, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTest35_AS(filename, goal="3'AS", toTest="QTL", threTest=8, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      rownameList <- c(rownameList, filename)
      cat(filename, "is tested\n")
