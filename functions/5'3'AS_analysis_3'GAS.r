...
-log10(0.05/nrow(g3asMatrix)/4)# = 3.48; 38 last exons were tested * 4 Env; => g3asThre=3.48
rm(g3asMatrix)
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/3'AS_chr", chr, "_GAS_wt_less.txt"))){
    ...
    if(length(gn_mixEnv) > 0) save(g3asGeneList, file = paste0("Data/geneticsAS/3'AS_chr", chr, "_GAS_genenameList.Rdata"))
