...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestGISE(filename, goal="skippingExon", P=2, toTest="QTL", threTest=8)
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
  ...
  if(!is.null(resmatrix)){
    ...
    write.table(resmatrix, file=paste0("Data/geneticsAS/GSE_chr", chr, "_ANOVA_less.txt"), sep="\t") #*********************** change with goal!!! ***********************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTestGISE(filename, goal="skippingExon", P=2, toTest="Int", threTest=11.6)
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      cat(filename, "is tested\n")
    }
  ...
  if(!is.null(resmatrix)){
    colnames(resmatrix) <- c("sepProbe", "sigMarker", "6H/gt1", "6H/gt2", "Dry_AR/gt1", "Dry_AR/gt2", "Dry_Fresh/gt1", "Dry_Fresh/gt2", "RP/gt1", "RP/gt2")
    write.table(resmatrix, file=paste0("Data/geneticsAS/ISE_chr", chr, "_ANOVA_less.txt"), sep="\t") #*********************** change with goal!!! ***********************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
  cat("chr", chr, "finished in", et-st, "s\n\n")
}
