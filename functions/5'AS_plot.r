...
plot_5AS <- function(filename, asThre=4.98){
  ...
  testAS <- read.table(paste0("Data/AS/5'AS_chr", chr, "_wt_less_p2.txt"), row.names=NULL)
  ...
  for(env in 1:4){
    ...
    if(env == 1){
      title(main=filename, cex.main=2.5, sub="5'AS by Wilcoxon Test", cex.sub=2, xlab="Probes", cex.lab=1.5, mgp=c(3, 0.5, 0), outer=TRUE)
    ...
    for(tu in uniqueExon){
      ...
      if(tu == uniqueExon[1]){
        rect(0.5, -3, fnPart[ ,2]+0.5, 20, density = 15, angle = 30, col = "azure2", border = "azure3")
        rect(fnPart[ ,2]+0.5, -3, max(ind)+0.5, 20, density = 15, angle = 150, col = "azure2", border = "azure3")
        
        #use median to test AS in 5'|3' site
        lines(c(0.5, fnPart[ ,2]+0.5), rep(median(unlist(newexp[exonID[exonID %in% 1:fnPart[ ,2]], ind_env])), 2), col=1, lty=2, lwd=1.5)
        
        if(fnPart[ ,env+2] >= asThre){
          text(x=(fnPart[ ,2]+max(ind)+1)/2, y=max(newexp)+0.1, labels=round(fnPart[ ,env+2], digits=1), col=env+1, cex=1)
          lines(c(fnPart[ ,2]+0.5, max(ind)+0.5), rep(median(unlist(newexp[exonID[exonID %in% (fnPart[,2]+1):max(ind)], ind_env])), 2), col=env+1, lwd=2)
        } else{
          if(round(fnPart[ ,env+2], digits=2) == asThre) text(x=(fnPart[ ,2]+max(ind)+1)/2, y=max(newexp)+0.1, labels=round(fnPart[ ,env+2], digits=1), col=1, cex=1)
          lines(c(fnPart[ ,2]+0.5, max(ind)+0.5), rep(median(unlist(newexp[exonID[exonID %in% (fnPart[,2]+1):max(ind)], ind_env])), 2), col=env+1, lty=2, lwd=1.5)
        }
...
asMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/AS/5'AS_chr", chr, "_wt_less_p2.txt"))){
    asMatrix <- rbind(asMatrix, read.table(paste0("Data/AS/5'AS_chr", chr, "_wt_less_p2.txt"), row.names=NULL))
...
for(filename in mostSig[1:5]){
  png(filename = paste0("Data/AS/plot/5'AS/", filename, "_5'AS_wt_mS.png"), width = 960, height = 1728, bg = "white")
  plot_5AS(filename, asThre=4.49)
  dev.off()
}
#plot around thre 5'AS
asMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/AS/5'AS_chr", chr, "_wt_less_p2.txt"))){
    asMatrix <- rbind(asMatrix, read.table(paste0("Data/AS/5'AS_chr", chr, "_wt_less_p2.txt"), row.names=NULL))
...
res <- NULL
for(e in 1:nrow(asMatrix)){
  res <- c(res, any(round(asMatrix[e, 3:6], digits=2) > (asThre-0.5) && round(asMatrix[e, 3:6], digits=2) < (asThre+0.5)))
}
aroundThre <- unique(asMatrix[res, 1])
for(filename in aroundThre[1:5]){
  png(filename = paste0("Data/AS/plot/5'AS/", filename, "_5'AS_wt_aT.png"), width = 960, height = 1728, bg = "white")
  plot_5AS(filename, asThre=4.49)
  dev.off()
}
...
#plot all 5'AS genes
for(chr in 1:5){
  load(file=paste0("Data/AS/5'AS_chr", chr, "_wt_p2.Rdata"))
  genenames <- seGeneList$mixEnv
  
  for(filename in genenames){
    png(filename = paste0("Data/AS/plot/5'AS/", filename, "_5'AS.png"), width = 960, height = 1728, bg = "white")
    plot_5AS(filename, asThre=4.49)
    dev.off()
  }
}
