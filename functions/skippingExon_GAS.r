...
#splicingTestSE_AS(filename, goal="skippingExon", toTest="QTL", threTest=8, useForTest=unlist, whichTest=wilcox.test, alternative="less", verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestSE_AS(filename, goal="skippingExon", toTest="QTL", threTest=8, useForTest=unlist, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTestSE_AS(filename, goal="skippingExon", toTest="Int", threTest=11.6, useForTest=unlist, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      cat(filename, "is tested\n")
    }
