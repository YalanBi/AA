...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTest35_AS(filename, goal="5'AS", toTest="QTL", threTest=8, useForTest=unlist, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
  ...
  if(!is.null(resmatrix)){
    ...
    write.table(resmatrix, file=paste0("Data/geneticsAS/5'AS_chr", chr, "_GAS_wt_less.txt"), sep="\t") #********** change!!! **********#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTest35_AS(filename, goal="3'AS", toTest="QTL", threTest=8, useForTest=unlist, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      rownameList <- c(rownameList, filename)
      cat(filename, "is tested\n")
