...
mapGenotypes <- function(chr=1, geno, menvironment){
  ...
  write.table(resEnv, file=paste("Data/fullModeMapping/expGenes_chr", chr, "_FM_Env.txt", sep=""), sep="\t")
  
  rownames(resQTL) <- colnames(expdata)
  colnames(resQTL) = colnames(geno)
  write.table(resQTL, file=paste("Data/fullModeMapping/expGenes_chr", chr, "_FM_QTL.txt", sep=""), sep="\t")
  
  rownames(resInt) <- colnames(expdata)
  colnames(resInt) = colnames(geno)
  write.table(resInt, file=paste("Data/fullModeMapping/expGenes_chr", chr, "_FM_Int.txt", sep=""), sep="\t")
