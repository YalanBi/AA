...
iseMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/SE_chr", chr, "_IAS_ANOVA_less.txt"))){
    iseMatrix <- rbind(iseMatrix, read.table(paste0("Data/geneticsAS/SE_chr", chr, "_IAS_ANOVA_less.txt"), row.names=NULL))
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/SE_chr", chr, "_IAS_ANOVA_less.txt"))){
    isechr <- read.table(paste0("Data/geneticsAS/SE_chr", chr, "_IAS_ANOVA_less.txt"), row.names=NULL)
    iseGeneList <- list()
    resmatrix <- NULL
    ...
    save(iseGeneList, file = paste0("Data/geneticsAS/SE_chr", chr, "_IAS_gnList_ANOVA.Rdata"))
  } else{
    cat("chr", chr, "NO test!\n")
