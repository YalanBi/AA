...
griMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/RI_chr", chr, "_GAS_ANOVA_less.txt"))){
    griMatrix <- rbind(griMatrix, read.table(paste0("Data/geneticsAS/RI_chr", chr, "_GAS_ANOVA_less.txt"), row.names=NULL))
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/RI_chr", chr, "_GAS_ANOVA_less.txt"))){
    grichr <- read.table(paste0("Data/geneticsAS/RI_chr", chr, "_GAS_ANOVA_less.txt"), row.names=NULL)
    griGeneList <- list()
    resmatrix <- NULL
    ...
    save(griGeneList, file = paste0("Data/geneticsAS/RI_chr", chr, "_GAS_gnList_ANOVA.Rdata"))
  } else{
    cat("chr", chr, "NO test!\n")
