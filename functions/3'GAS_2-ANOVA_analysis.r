...
g3asMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/3'AS_chr", chr, "_GAS_ANOVA_less.txt"))){
    g3asMatrix <- rbind(g3asMatrix, read.table(paste0("Data/geneticsAS/3'AS_chr", chr, "_GAS_ANOVA_less.txt"), row.names=NULL))
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/3'AS_chr", chr, "_GAS_ANOVA_less.txt"))){
    g3aschr <- read.table(paste0("Data/geneticsAS/3'AS_chr", chr, "_GAS_ANOVA_less.txt"), row.names=NULL)
    g3asGeneList <- list()
    resmatrix <- NULL
    ...
    if(length(gn_mixEnv) > 0) save(g3asGeneList, file = paste0("Data/geneticsAS/3'AS_chr", chr, "_GAS_gnList_ANOVA.Rdata"))
