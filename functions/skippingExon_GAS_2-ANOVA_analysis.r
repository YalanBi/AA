...
gseMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/SE_chr", chr, "_GAS_ANOVA_less.txt"))){
    gseMatrix <- rbind(gseMatrix, read.table(paste0("Data/geneticsAS/SE_chr", chr, "_GAS_ANOVA_less.txt"), row.names=NULL))
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/SE_chr", chr, "_GAS_ANOVA_less.txt"))){
    gsechr <- read.table(paste0("Data/geneticsAS/SE_chr", chr, "_GAS_ANOVA_less.txt"), row.names=NULL)
    gseGeneList <- list()
    resmatrix <- NULL
    ...
    save(gseGeneList, file = paste0("Data/geneticsAS/SE_chr", chr, "_GAS_gnList_ANOVA.Rdata"))
  } else{
    cat("chr", chr, "NO test!\n")
