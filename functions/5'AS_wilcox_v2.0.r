...
#5' AS test
splicingTestAt5 <- function(filename, P=2, verbose=FALSE){
  ...
  if(length(uniqueExon) >= 2 && length(exonID) >= (2*P+1) && length(ind) >= 2*P && !is.null(restProbes)){
    if(verbose) cat(filename, "have", length(uniqueExon), "exons,", length(exonID), "exonProbes,", length(ind), "exonProbes in firstExon and have expExons, can be tested for 5'AS!\n")
    
    sepPoint <- findSepPoint(toGroup=ind, exp_data=rawexp[ ,17:164], verbose=FALSE)
    if(!is.null(sepPoint) && length(ind[1:sepPoint]) >= P && length(ind[-(1:sepPoint)]) >= P && median(unlist(rawexp[ind[1:sepPoint], 17:164])) >= 5 && median(unlist(rawexp[ind[-(1:sepPoint)], 17:164])) < 5){
      if(verbose) cat(" =>separate between p", ind[sepPoint], "and p", ind[sepPoint+1], ":",
      "partI(", length(ind[1:sepPoint]), ") median is", median(unlist(rawexp[ind[1:sepPoint], 17:164])), "; partII(", length(ind[-(1:sepPoint)]), ") median is", median(unlist(rawexp[ind[-(1:sepPoint)], 17:164])),
      "\nready for test!\n")
  ...
  } else if(verbose) cat("we don't have enough exons/exonProbes/exonProbes in firstExon/no expExon T^T\n")
}
#splicingTestAt5(filename, P=2, verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestAt5(filename, P=2)
    if(!is.null(res)){
  ...
  if(!is.null(resmatrix)){
    ...
    write.table(resmatrix, file=paste0("Data/AS/5'AS_chr", chr, "_wt_less_p2.txt"), sep="\t") #**************************** change with goal!!! ****************************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
