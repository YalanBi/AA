...
g5asMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/5'AS_chr", chr, "_GAS_ANOVA_less.txt"))){
    g5asMatrix <- rbind(g5asMatrix, read.table(paste0("Data/geneticsAS/5'AS_chr", chr, "_GAS_ANOVA_less.txt"), row.names=NULL))
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/5'AS_chr", chr, "_GAS_ANOVA_less.txt"))){
    g5aschr <- read.table(paste0("Data/geneticsAS/5'AS_chr", chr, "_GAS_ANOVA_less.txt"), row.names=NULL)
    g5asGeneList <- list()
    resmatrix <- NULL
    ...
    if(length(gn_mixEnv) > 0) save(g5asGeneList, file = paste0("Data/geneticsAS/5'AS_chr", chr, "_GAS_gnList_ANOVA.Rdata"))
