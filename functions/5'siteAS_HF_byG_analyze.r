...
#************************************** this is the final version for analyzing results of testing genetics regulated AS 5'site ^_^ **************************************#
...
for(chr in 1:5){
  gaschr <- read.table(paste0("Data/geneticsAS/splicing5'siteByG_chr", chr, "_wt_p2.txt"), row.names=NULL)
  gasGeneList <- list()
  resmatrix <- NULL
  ...
  if(length(gn_mixEnv) > 0) save(gasGeneList, file=paste0("Data/geneticsAS/splicing5'siteByG_chr", chr, "_wt_p2.Rdata"))
}
