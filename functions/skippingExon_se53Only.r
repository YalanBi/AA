...
#************************************ this is the final version for testing whether the firsrt/last exons are spliced out or not! ^_^ ************************************#
...
splicingTestSE <- function(filename, goal="skipping53Exon", verbose=FALSE, ...){
...
#splicingTestSE(filename, goal="skipping53Exon", useForTest=unlist, whichTest=wilcox.test, alternative="less", verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTestSE(filename, goal="skipping53Exon", useForTest=unlist, whichTest=wilcox.test, alternative="less")
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
  ...
  write.table(resmatrix, file=paste0("Data/skippingExon/SE_chr", chr, "_se53Only_wt_less.txt"), sep="\t") #********** change!!! **********#
