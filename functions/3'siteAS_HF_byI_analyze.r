...
#************************************ this is the final version for analyzing results of testing interaction regulated AS 3'site ^_^ *************************************#
#******************************************************************** testing algorithm: Wilcox.test! ********************************************************************#

setwd("D:/Arabidopsis Arrays")

#calculate the threshold for interaction regulated 3'site alternative splicing
iasMatrix <- NULL
for(chr in 1:5){
  iasMatrix <- rbind(iasMatrix, read.table(paste0("Data/geneticsAS/splicing3'siteByI_chr", chr, "_wt_p2.txt"), row.names=NULL))
}
...
for(chr in 1:5){
  iaschr <- read.table(paste0("Data/geneticsAS/splicing3'siteByI_chr", chr, "_wt_p2.txt"), row.names=NULL)
  iasGeneList <- list()
  resmatrix <- NULL
  ...
  if(length(gn_mixEnv) > 0) save(iasGeneList, file=paste0("Data/geneticsAS/splicing3'siteByI_chr", chr, "_wt_p2.Rdata"))
}
