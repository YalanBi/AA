...
as5Matrix <- NULL
for(chr in 1:5){
  as5Matrix <- rbind(as5Matrix, read.table(paste0("Data/AS/5AS_chr", chr, "_ANOVA_less.txt"), row.names=NULL))
}
...
for(chr in 1:5){
  as5chr <- read.table(paste0("Data/AS/5AS_chr", chr, "_ANOVA_less.txt"), row.names=NULL)
  as5GeneList <- list()
  ...
  save(as5GeneList, file = paste0("Data/AS/5AS_chr", chr, "_gnList_ANOVA.Rdata"))
}
