...
-log10(0.05/nrow(g5asMatrix)/4)# = 3.46; 36 first exons were tested * 4 Env; => g5asThre=3.46
rm(g5asMatrix)
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/5'AS_chr", chr, "_GAS_wt_less.txt"))){
    ...
    if(length(gn_mixEnv) > 0) save(g5asGeneList, file = paste0("Data/geneticsAS/5'AS_chr", chr, "_GAS_genenameList.Rdata"))
