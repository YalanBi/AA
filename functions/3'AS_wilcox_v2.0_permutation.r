...
#3' AS test
permutation3AS <- function(filename, P=2, verbose=FALSE){
  ...
  ind <- exonID[rawexp[exonID, "tu"] == uniqueExon[length(uniqueExon)]]
  ...
  if(length(uniqueExon) >= 2 && length(exonID) >= (2*P+1) && length(ind) >= 2*P){
    if(verbose) cat(filename, "have", length(uniqueExon), "exons,", length(exonID), "exonProbes, and", length(ind), "exonProbes in lastExon, can be tested for 3'AS!\n")
    ...
    if(!is.null(restProbes)){
      ...
      ind <- sort(exonID[rawexp[exonID, "tu"] == uniqueExon[length(uniqueExon)]], decreasing=TRUE) #new exonProbes in last exon and turn it over
      sepPoint <- findSepPoint(toGroup=ind, exp_data=rawexp[ ,17:164], verbose=FALSE)
      if(!is.null(sepPoint) && sepPoint > 1 && sepPoint < (length(ind)-1) && median(unlist(rawexp[ind[sepPoint:1], 17:164])) >= 5 && median(unlist(rawexp[ind[-(1:sepPoint)], 17:164])) < 5){
        ...
        return(max(res))
      } else if(verbose) cat(" =>last exon decreases/not enough exonProbes in each part/not median>=5 + median<5 after separation T^T\n")
    } else if(verbose) cat(" after permutation, no expExons T^T\n")
  } else if(verbose) cat(filename, "don't have enough exons/exonProbes/exonProbes in lastExon T^T\n")
}
#permutation3AS(filename, P=2, verbose=TRUE)
...
for(r in 1:1000){
  ...
  while(n <= testRange){
    ...
    res <- permutation3AS(filename, P=2)
    if(!is.null(res)){
...
write.table(maxList, file="Data/AS/Permutation for 3'AS.txt", row.names=FALSE)


png(filename="Data/AS/Histogram of Permutation for 3'AS.png", width=480, height=480, bg="White")
hist(maxList, breaks=50, border=grey(0.5), col="grey", main="Histogram of Results of Permutation for Alternative 3' Splicing Site", xlab="")
abline(v=65, col='red')
axis(1, at=65, labels=65, col='red', col.axis='red')
dev.off()
