...
splicingTest35 <- function(filename, goal="5'AS", P=2, verbose=FALSE){
  ...
  if(length(uniqueExon) >= 2 && length(exonID) >= (2*P+1)){
    ...
    if(length(ind) >= 2*P){
      ...
      #>= P probes left in each group of the first exon, remember the gene name and do ANOVA
...
#splicingTest35(filename, goal="5'AS", P=2, verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- splicingTest35(filename, goal="5'AS", P=2)
    if(!is.null(res)){
  ...
  if(!is.null(resmatrix)){
    ...
    write.table(resmatrix, file=paste0("Data/AS/5'AS_chr", chr, "_ANOVA_less.txt"), sep="\t") #*************************** change with goal!!! ***************************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
...
for(chr in 1:5){
  ...
  #filename = "AT1G01010"
  for(filename in genenames){
    res <- splicingTest35(filename, goal="3'AS", P=2)
    if(!is.null(res)){
      resmatrix <- rbind(resmatrix, res)
      rownameList <- c(rownameList, filename)
      cat(filename, "is tested\n")
  ...
  if(!is.null(resmatrix)){
    rownames(resmatrix) <- rownameList
    colnames(resmatrix) <- c("sepProbe", "6H", "Dry_AR", "Dry_Fresh", "RP")
    write.table(resmatrix, file=paste0("Data/AS/3'AS_chr", chr, "_ANOVA_less.txt"), sep="\t") #*************************** change with goal!!! ***************************#
  } else cat("\tNO TEST!\n")
  et <- proc.time()[3]
  cat("chr", chr, "finished in", et-st, "s\n\n")
}
