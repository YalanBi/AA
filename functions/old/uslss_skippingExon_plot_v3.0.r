...
plot_SE <- function(filename, seThre=6){
  ...
  testAS <- read.table(paste0("Data/AS/SE_chr", chr, "_Ex+In.txt"), row.names=NULL)
  fnPart <- testAS[grepl(filename, testAS[ ,1]), ]
  ...
  for(env in 1:4){
    ...
    for(tu in uniqueExon){
      ...
      if(length(e) > 0){
        if(fnPart[e, 2*env+1] < seThre && fnPart[e, 2*env+2] >= seThre){
          text(x=mean(ind), y=max(newexp)+0.1, labels=paste0(round(fnPart[e, 2*env+1], digits=1), "/", round(fnPart[e, 2*env+2], digits=1)), col=env+1, cex=1)
          lines(c(min(ind)-0.5, max(ind)+0.5), rep(mean(unlist(newexp[exonID[exonID %in% ind], ind_env])), 2), col=env+1, lwd=2)
        } else{
          if(any(round(fnPart[e, (2*env+1):(2*env+2)], digits=2) == seThre)) text(x=mean(ind), y=max(newexp)+0.1, labels=paste0(round(fnPart[e, 2*env+1], digits=1), "/", round(fnPart[e, 2*env+2], digits=1)), col=1, cex=1)
...
seMatrix <- NULL
for(chr in 1:5){
  if(file.exists(paste0("Data/AS/SE_chr", chr, "_vsIn_greater.txt"))){
    seMatrix <- rbind(seMatrix, read.table(paste0("Data/AS/SE_chr", chr, "_vsIn_greater.txt"), row.names=NULL))
...
mostSig <- unique(seMatrix[order(res, decreasing=FALSE), 1])
for(filename in mostSig[1:5]){
  png(filename = paste0("Data/AS/plot/SE/", filename, "_SE_vsIn_mS.png"), width = 960, height = 1728, bg = "white")
  plot_SE(filename, eThre=6)
  dev.off()
}
#plot around thre SE
seMatrix <- NULL
...
for(filename in aroundThre[1:5]){
  png(filename = paste0("Data/AS/plot/SE/", filename, "_SE_vsIn_aT.png"), width = 960, height = 1728, bg = "white")
  plot_SE(filename, seThre=6)
  dev.off()
}
