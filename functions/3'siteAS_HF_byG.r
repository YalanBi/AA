...
#*********************************************** this is the final version for testing genetics regulated AS at 3 site ^_^ ***********************************************#
...
#3'site GAS test
test3siteGAS <- function(filename, threTest=8, P=2, verbose=FALSE){
  ...
  #for genetics regulated alternative splicing 3'site, at least 2 exons in a gene!!!
...
#test3siteGAS(filename, threTest=8, P=2, verbose=TRUE)
...
for(chr in 1:5){
  ...
  for(filename in genenames){
    res <- test3siteGAS(filename, threTest=8, P=2)
  ...
  write.table(resmatrix, file=paste0("Data/geneticsAS/splicing3'siteByG_chr", chr, "_wt_p2.txt"), sep="\t")
  et <- proc.time()[3]
