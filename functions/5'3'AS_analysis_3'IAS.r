...
-log10(0.05/nrow(i3asMatrix)/4)# = 2.60; 5 last exons were tested * 4 Env; => i3asThre=2.60
rm(i3asMatrix)
...
for(chr in 1:5){
  if(file.exists(paste0("Data/geneticsAS/3'AS_chr", chr, "_IAS_wt_less.txt"))){
    ...
    if(length(gn_mixEnv) > 0) save(i3asGeneList, file = paste0("Data/geneticsAS/3'AS_chr", chr, "_IAS_genenameList.Rdata"))
