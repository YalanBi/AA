...
#************************************************************** mapping part **************************************************************#
#Never use anymore!!!
#due to removing SNPs, expressed genes changed. Remove unexpressed files and mapping again for expressed genes without QTL files
load(file="Data/fullModeMapping/expGenes.Rdata")

for(chr in 1:5){
  location_FM <- paste0("Data/fullModeMapping/chr", chr, "_norm_hf_cor/")
  oldexpgenes <- dir(location_FM)[grepl("QTL", dir(location_FM))]
  newexpgenes <- expGeneList[[chr]]
  
  notexistgenes <- gsub("_QTL.txt", "", oldexpgenes[!gsub(".FM_QTL", "", oldexpgenes) %in% newexpgenes])
  for(kind in c("_Env", "_QTL", "_Int")){
    file.rename(paste0(location_FM, paste0(notexistgenes, kind, ".txt")), paste0("not exist in expGenes/", paste0(notexistgenes, kind, ".txt")))
  }
}
